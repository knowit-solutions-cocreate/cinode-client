...
def cli_api(monkeypatch: pytest.MonkeyPatch) -> Iterator[respx.MockRouter]:
    ...
    Retries sleep for nothing, so a 429 that outlasts them costs no time.
    """
    ...
    kwdefaults = Transport.__init__.__kwdefaults__
    assert kwdefaults is not None
    monkeypatch.setitem(kwdefaults, "sleep", lambda _seconds: None)  # pyright: ignore[reportUnknownLambdaType]
