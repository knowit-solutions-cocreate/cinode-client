...
| **v0.2** | In progress ([plan](plan.md)) | User profiles and resumes (`users.profile.get`, `users.resumes.list/get`, `ops.team_profiles`) |
| v0.3 | Later | Output for humans (`--table`); more reads as needed: user roles, team managers, keyword lookups |
